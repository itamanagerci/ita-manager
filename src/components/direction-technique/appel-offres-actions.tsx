...
import { refuserAppelOffres, validerAppelOffres } from "@/lib/server-actions/appel-offres";
...
interface AppelOffresActionsProps {
  demandeId: string;
}

export function AppelOffresActions({ demandeId }: AppelOffresActionsProps) {
  ...
  async function valider() {
    setEnCours(true);
    const resultat = await validerAppelOffres(demandeId);
    setEnCours(false);
    ...
    notifier.succes("Appel d'offres validé");
    router.refresh();
  }
  ...
  async function confirmerRefus() {
    setErreur(null);
    setEnCours(true);
    const resultat = await refuserAppelOffres(demandeId, motif);
    setEnCours(false);
    ...
    setMotif("");
    notifier.succes("Appel d'offres refusé");
    router.refresh();
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Dialog open={dialogRefus} onOpenChange={setDialogRefus}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Refuser cet appel d&apos;offres</DialogTitle>
          </DialogHeader>
