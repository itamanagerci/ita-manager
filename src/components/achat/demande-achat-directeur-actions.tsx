...
interface DemandeAchatDirecteurActionsProps {
  demandeId: string;
}

export function DemandeAchatDirecteurActions({ demandeId }: DemandeAchatDirecteurActionsProps) {
  ...
  async function valider() {
    setEnCours(true);
    const resultat = await validerDemandeAchatDirecteur(demandeId);
    setEnCours(false);
    ...
    notifier.succes("Demande validée", "Transmise au Responsable Achat.");
    router.refresh();
  }
  ...
  async function confirmerRefus() {
    setErreur(null);
    setEnCours(true);
    const resultat = await refuserDemandeAchatDirecteur(demandeId, motif);
    setEnCours(false);
