...
import { refuserAbsence, validerAbsence } from "@/lib/server-actions/rh-absences";
...
export function AbsenceActions({ demandeId }: AbsenceActionsProps) {
  ...
  async function valider() {
    setEnCours(true);
    const resultat = await validerAbsence(demandeId);
    setEnCours(false);
  ...
  async function confirmerRefus() {
    setErreur(null);
    setEnCours(true);
    const resultat = await refuserAbsence(demandeId, motif);
    setEnCours(false);
